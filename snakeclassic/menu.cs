...
namespace snakeclassic
{
    ...
    {
        // Один экземпляр настроек — не пересоздаём каждый раз!
        private nastoy nastoyForm = null;
        ...
        {
            // Используем один и тот же экземпляр — не пересоздаём!
            if (nastoyForm == null || nastoyForm.IsDisposed)
                nastoyForm = new nastoy();
